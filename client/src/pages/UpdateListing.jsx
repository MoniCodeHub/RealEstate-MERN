...
const UpdateListing = () => {
    ...
    useEffect(() => {
        const fetchListing = async () => {
            ...
            const res = await fetch(`/api/listing/getForEdit/${listingId}`);
        ...
        <form onSubmit={handleSubmit} className='flex flex-col sm:flex-row  gap-4 '>
            <div className='flex flex-col gap-4 flex-1'>
                ...
                <div className='flex flex-wrap gap-3'>
                    ...
                    <div className='flex items-center gap-2'>
                        <input type='number' id='regularPrice' min='500' max='300000' required
                        className='p-3 border border-gray-300 rounded-lg'
                        onChange={handleChange}
            ...
            <div className='flex flex-col flex-1'>
                ...
                <div className='flex my-4 gap-4'>
                    ...
                    className='p-3 border border-fuchsia-500 rounded w-full' />
                    <button
                    type='button' disabled={uploading}
                    onClick = {handleImageSubmit}
                    className='p-3 text-blue-700 border border-blue-400 font-bold
                ...
                <p className='text-red-500 text-sm'>{imageUploadError && imageUploadError}</p>
                {
                    formData.imageUrls.length > 0 && formData.imageUrls.map((url , index)=> (
                        ...
                        <div key={url} className='flex justify-between p-3 border items-center'>
                            <img src={url} className='w-30 h-20 object-cover border  border-fuchsia-500 rounded-md' alt="listing image"  />
                            <button type='button' onClick={() => handleRemoveImage(index)}
                             className='p-3 text-red-700 rounded-lg uppercase hover:opcaity-80'>Delete</button>
                        </div>
