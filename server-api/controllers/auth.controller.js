...
export const signup = async (req, res, next) => {
  ...
  try {
    ...
    next(errorHandler(550, "Error due to functional error"));
  }
};
...
export const signin = async (req, res, next) => {
  ...
  try {
    ...
    const { password: pass, ...rest } = validUser._doc; //we do this to hide the password being visible in console
...
export const google = async (req, res, next) => {
  try {
    ...
    if (user) {
      ...
      //google auth doesn't need pswd to be entered by user; so we generate which user can modify later
